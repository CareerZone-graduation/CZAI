    ...
    for exp in experiences:
        ...
        try:
            start_date = datetime.fromisoformat(
                start_str) if isinstance(start_str, str) else start_str
            if end_str:
                end_date = datetime.fromisoformat(
                    end_str) if isinstance(end_str, str) else end_str
            else:
...
async def get_candidate_recommendations(
    ...
    if not chunks:
        raise HTTPException(
            status_code=400, detail="Job has no embeddings yet (chunks missing)")

    valid_embeddings = [c.get("embedding")
                        for c in chunks if c.get("embedding")]
    if not valid_embeddings:
        raise HTTPException(
            status_code=400, detail="Job has no valid embeddings")
    ...
    if not matched_users:
        return CandidateRecommendationResponse(
            ...
            pagination={"currentPage": page, "totalPages": 0, "totalItems": 0,
                        "limit": limit, "hasNextPage": False, "hasPrevPage": False},
    ...
    job_skills_lower = [s.lower().strip()
                        for s in job_skills_list if isinstance(s, str)]
    ...
    for user in matched_users:
        ...
        if job_skills_list and prof_skills:
            candidate_skills_lower = [s.get("name", "").lower().strip(
            ) for s in prof_skills if isinstance(s, dict) and s.get("name")]
            exact_matches = 0
            partial_matches = 0
            matched_skill_names = []
            ...
            for c_skill in candidate_skills_lower:
                if c_skill in job_skills_lower:
                    ...
                    has_partial = any(
                        js in c_skill or c_skill in js for js in job_skills_lower)
                    if has_partial:
                        partial_matches += 1
        ...
        if job_province and profile.get("preferredLocations"):
            loc_match = next((loc for loc in profile.get(
                "preferredLocations", []) if loc.get("province") == job_province), None)
            if loc_match:
                loc_score = 7
                ...
                dist_str = f", {loc_match.get('district')}" if loc_match.get(
                    "district") else ""
                match_reasons.append({
                    "type": "location_match",
        ...
        if normalized_score >= minScore:
            scored_candidates.append(CandidateScore(
                ...
                matchedSkills=extract_matched_skills(
                    job_skills_list, prof_skills),
                experienceYears=calculate_experience_years(
...
# ── Get similar jobs via Item-Item CF ─────────────────────────────────────────


@router.get("/recommendation/similar-jobs-cf/{job_id}", response_model=SimilarJobCFResponse)
async def get_similar_jobs_cf(
    job_id: str,
    limit: int = 6,
    ...
    """
    Gợi ý việc làm bằng Item-Item Collaborative Filtering (LightFM).

    Trả về "việc làm mà người có cùng sở thích với bạn cũng quan tâm":
    LightFM item embeddings encode collaborative signal từ interaction data,
    nên jobs có embedding gần nhau = được cùng nhóm users quan tâm.

    - **job_id**: Job đang xem (target)
    - **limit**: Số lượng kết quả (default 6)
    - **userId**: (Optional) Nếu truyền, exclude jobs user đã tương tác
    """
    # _verify_internal(x_internal_secret)

    if not engine.is_ready:
        results, source = engine._popular_fallback(limit)
        return SimilarJobCFResponse(
            jobId=job_id, data=[JobScore(**r) for r in results], source=source
        )

    loop = asyncio.get_running_loop()
    results, source = await loop.run_in_executor(
        _executor,
        lambda: engine.get_similar_items(
            job_id=job_id, n=limit, user_id=userId),
    )

    return SimilarJobCFResponse(
        jobId=job_id,
        data=[JobScore(**r) for r in results],
        source=source,
    )
